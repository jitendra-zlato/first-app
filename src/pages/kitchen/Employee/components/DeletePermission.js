...
import { deleteEmployeePermission } from "../../../../actions/kitchen/employee-profile";
...
const AddPermission = ({ kitchenId, employeeId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    dispatch(deleteEmployeePermission(formData));
  };
  ...
  useEffect(() => {
    if (isdeleted) {
      notifySuccess("Employee permission deleted.");
    ...
    <div className="w-[500px] max-w-full mx-auto px-4 py-8 sm:py-12">
      <div className="p-4 w-full flex flex-col shadow-lg rounded bg-white sm:p-8">
        <div className="w-full mb-8">
          <h1 className="text-2xl font-medium mb-2">Delete Permission</h1>
        </div>
        <form className="w-full" onSubmit={handleSubmit}>
          ...
          <button className="w-full text-center h-10 p-2 bg-cyan-600 text-white select-none cursor-pointer">
            {loading ? <DotPulse /> : <>Delete Permission</>}
          </button>
        </form>
      </div>
    </div>
